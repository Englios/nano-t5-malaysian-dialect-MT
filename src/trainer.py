...
def main():
    ...
    if "wandb" in (training_args.report_to or []):
        try:
            ...
            if not wandb.api.api_key:
                ...
                logger.error("=" * 80)
                raise RuntimeError(
                    "Wandb login required. Run 'wandb login' to authenticate."
                )
            ...
            if wandb.run is None:
                ...
                wandb_run_name = (
                    training_args.run_name if hasattr(training_args, "run_name") else None
                )
            ...
            # Override hyperparameters from W&B Sweeps if they exist
            if wandb.config:
                sweep_params = {}
            ...
            logger.error(
                "Wandb logging is required. Please fix the issue and try again."
            )
            raise
    ...
    if training_args.do_predict and not training_args.do_train:
        if os.path.exists(training_args.output_dir) and os.path.exists(
            ...
            logger.warning(
                f"No trained model found in {training_args.output_dir}. Using base model: {model_path}"
            )
